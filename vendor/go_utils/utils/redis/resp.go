...
func (r *RedisClient) SimpleCommandSingleReturnNoLock(cmd ...string) (string, error) {
	if r.conn == nil {
		if err := r.DialRedis(); err != nil {
			return "", err
		}
...
func (r *RedisClient) SimpleCommandMultiReturn(cmd ...string) ([]string, error) {
	...
	var retArray []string
	if ret, err := r.respSendRawCommand(strings.Join(cmd, " ")); err == nil {
		utils.LogPrintDebug4("ret: ", string(ret))
		utils.LogPrintDebug4("ret byte: ", ret)
		utils.LogPrintDebug4("ret len: ", len(ret))
		if len(ret) == 0 {
			return []string{}, nil
		}
		...
		utils.LogPrintDebug4("retArray:", retArray)
		utils.LogPrintDebug4("retArray len:", len(retArray))
		return retArray, nil
	} else {
		return []string{}, err
	}
